...
// 将预生成的ID替换，否则当有两个Component挂在同一个树上时，后一个会将前一个的属性覆盖
        ...
        for (const key in object) {
            if (object.hasOwnProperty(key)) {
                ...
                if (key === "lc_id" && object.hasOwnProperty("__key__")) {
                    ...
                    // 这个备份用于生成兄弟Component时获取原始代码所用，需要保留ID的信息才可以实现DOM与代码对象的关联
...
export function generateRawInfo(target) {
    if (target.attributes.lc_id) {
        ...
        // 代表这是一个VueComponent组成的元素，这里的逻辑渐渐不再使用
...
/**
 * 寻找实际的可以代表整个复合ComponentDom，这是个核心方法，根据某个元素查找实际的以VueComponent为单位的最小元素
 */
